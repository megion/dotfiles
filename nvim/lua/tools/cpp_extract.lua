...
local STRIP_DECL_KEYWORDS =
  { "static", "inline", "virtual", "friend", "explicit", "override" }

local function notify(msg, level)
  vim.notify(
    msg,
    level or vim.log.levels.INFO,
    { title = "CppExtractDefinitions" }
  )
end
...
local function find_class_body(class_node)
  return get_field_child(class_node, "body")
    or find_first_named_child(class_node, "field_declaration_list")
end
...
local function build_declaration(bufnr, function_node)
  ...
  local initializer_list =
    find_direct_named_child(function_node, "field_initializer_list")
...
local function build_definition_text(bufnr, function_node, scope_prefix)
  local prefix, err =
    build_definition_prefix(bufnr, function_node, scope_prefix)
  if not prefix then
    return nil, err
  end
...
local function find_namespace_insert_index(lines, namespace_parts)
  ...
  for index, line in ipairs(lines) do
    ...
    if decl_parts then
      ...
      if
        namespace_parts_equal(
          flatten_namespace_stack(namespace_stack),
          namespace_parts
        )
      then
        target_entry = entry
    ...
    while
      #namespace_stack > 0
      and brace_depth == namespace_stack[#namespace_stack].close_depth
    do
...
local function replace_function_with_declaration(
  bufnr,
  function_node,
  declaration
)
...
local function append_missing_definitions(
  cpp_path,
  include_line,
  definition_entries,
  namespace_parts
  ...
  for _, entry in ipairs(definition_entries) do
    local has_local = normalized_existing:find(entry.local_signature, 1, true)
      ~= nil
    local has_full = normalized_existing:find(entry.full_signature, 1, true)
      ~= nil
  ...
  if appended > 0 then
    ...
    if insert_at then
      ...
      vim.list_extend(
        lines,
        wrap_in_namespaces(namespace_parts, new_definitions)
      )
    end
    changed = true
  end
...
local function build_definition_entry(
  bufnr,
  function_node,
  declaration_builder,
  scope_prefix,
  ...
  local definition, err =
    build_definition_text(bufnr, function_node, scope_prefix)
  if not declaration or not definition then
    return nil, err
  end

  local local_signature =
    normalize_ws(definition:match("^(.-)%s*%b{}") or definition)
  local full_definition =
    build_definition_text(bufnr, function_node, full_scope_prefix)
  local full_signature = normalize_ws(
    (full_definition or definition):match("^(.-)%s*%b{}")
      or (full_definition or definition)
  )
...
local function extract_class_functions(functions, class_node, bufnr, path)
  ...
  if vim.tbl_isempty(functions) then
    notify(
      "No non-template inline member definitions found in the current class",
      vim.log.levels.INFO
    )
    return
  end
  ...
  local full_scope_parts =
    vim.list_extend(vim.deepcopy(namespace_parts), { class_name })
  ...
  for _, function_node in ipairs(functions) do
    ...
    if entry then
      ...
      table.insert(
        replacements,
        { node = entry.node, declaration = entry.declaration }
      )
    elseif err then
  ...
  local appended = append_missing_definitions(
    cpp_path,
    include_line,
    definition_entries,
    namespace_parts
...
local function extract_namespace_function(function_node, bufnr, path)
  ...
  local name_node =
    resolve_function_name_node(get_field_child(function_node, "declarator"))
  local function_name = name_node and trim(get_node_text(name_node, bufnr))
    or ""
...
function M.extract_current_function()
  ...
  if not function_node then
    notify(
      "Place the cursor inside the function definition to extract",
  ...
  if has_template_ancestor(function_node) then
    notify(
      "Template function definitions should stay in the header",
      vim.log.levels.WARN
    )
    return
  end
